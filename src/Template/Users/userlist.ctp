 ...
 <?php echo $this->element('admin_header'); ?>
        <?php echo $this->element('admin_sidebar');
            ...
            $account_status = [SUSPEND=>'Suspend',RESUME=>'Resume'];
            $btnAccount_status = [RESUME=>'btn-success',SUSPEND=>'btn-danger'];
         ?>
    <!--  PAPER WRAP -->
    <div class="wrap-fluid">
        <div class="container-fluid paper-wrap bevel tlbr">
            ...
            <ul id="breadcrumb">
                ...
                <li><a href="javascript:;" title="Sample page 1">User List</a>
                </li>
            ...
            <div class="content-wrap">
                <div class="row">
                    ...
                    <div class="col-sm-12">
						...
						<?php echo $this->Html->image('loading.gif',['class'=>'DisplayNone']); ?>
                                               <!--      <button type="button" class="btn btnSuspend <?php //echo $btnAccount_status[$val->suspend_account] ?>" data-id="<?php //echo $val->id; ?>" data-status="<?php //echo $val->suspend_account; ?>">
                                                     <?php //echo $account_status[$val->suspend_account]; ?></button>-->
...
$(document).ready(function(){
    $('.btnDelete').click(function(){
		...
		location.reload();
		//window.location.href='<?php echo SITE_URL.'users/userlist';?>';
               }
            });
   ...
   $(function(){
       ...
       $('.btnSuspend').click(function(){
         var id = $(this).data('id');
        var account_suspend = $(this).data('status');
         if(confirm('Do you want to Suspend?')){
            $.ajax({
               type:'POST',
               data:{id:id,account_suspend,account_suspend},
               url:"<?php echo SITE_URL.'Users/suspend';?>",
               success:function(data) {
               }
            });
             if(account_suspend == '<?php echo SUSPEND; ?>'){
               $(this).removeClass('btn-danger');
               $(this).addClass('btn-success');
               $(this).text('Resume');
               $(this).data('account_suspend','<?php echo RESUME; ?>');
            }else{
               $(this).removeClass('btn-success');
               $(this).addClass('btn-danger');
               $(this).text('Suspend');
               $(this).data('account_suspend','<?php echo SUSPEND; ?>');
            }
           // $(this).closest('tr').remove();
         }
